...
module button(length, width, thickness, height)
{
    ...
    translate([length/2-width/2,0,0]) cylinder(h=height, r=thickness, $fn=60);

    translate([(length/2+width/2),-thickness/2,0])cube([(length/2-width/2), thickness, height]);
    
    translate([length/2+width/2,0,0]) cylinder(h=height, r=thickness, $fn=120);

    translate([length/2-3*width/4,width/3,0]) cube([1.5*width, thickness, height]);
    
    translate([length/2-width/2,2*width/3,0]) cube([width, thickness, height]);
    
    translate([length/2-width/2,width/3,0]) cube([thickness, width/3, height]);

    translate([length/2+width/2-thickness,width/3,0]) cube([thickness, width/3, height]);
