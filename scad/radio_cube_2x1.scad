...
module radio_cube_2x1(width, height, thickness)
{
    ...
    difference()
    {
        cube([width2,width,height]);
        union()
        {
            translate([thickness,thickness,thickness+1.2]) cube([width2-2*thickness,width-2*thickness,height+10]);
            ...
            translate([3*width/2+1,width+0.6,8])rotate([90,0,0])cylinder(r=14.5/2, h=2.6);
            translate([3*width/2+1,width-thickness,8])rotate([90,0,0])cylinder(r=14.5/2, h=1);
            translate([3*width/2-3, width-10,5.8])cube([8,20,15]);
