...
module cap_1x1_transformer(width, height, thickness)
{
    difference()
    {
        cap_1x1(width, height, thickness, 0); //15
        union()
        {
            translate([width/2, -width/4+glyph_thickness, -0.01]) rotate([0,0,45])transformer_1_1(width, glyph_depth, glyph_thickness);
            ...
            //#upper skew-line        
            //translate([9*width/16,3*width/16+0.3,-0.01])rotate([0,0, 135])cube([glyph_thickness, width/16, glyph_depth]);
